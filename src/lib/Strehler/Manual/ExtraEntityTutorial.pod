...
=head1 RAISE OF THE ROBOTS

So you have installed Strehler, but you need more power, you need a CRUD for something more specific than articles and images, but you want it in Strehler flavour. Usually this kind of tutorial uses books or CDs as example, but these are boring things. We'll create a collection of bad-ass giant robots.
...
We'll trace also robots' descriptions, but as we will manage them as multilanguage field we'll talk about them in a second moment.

Considering this data structure you have to create database table for it by yourself, Strehler doesn't give you tools to manage database table out of the ones for tables of its own.
...
Table fields are more than data we asked for. ID si mandatory in B<every> Strehler entity table as primary key for it. We also decided to give this object two Strehler out-of-the-box featuers: published status and category. Designing the database is enough to add a CATEGORY column and a PUBLISHED one (INTEGER and TINYINT respectivly).
...
Strehler do no introspection to elaborate the form to create/edit robots so we have to write it by ourself. It's just HTML::FormFu syntax. You can do everything you want, just remember to keep fields names the same of the database fields names. For Strehler standard features use custom FormFu::Elements.
...
Classes are assigned according to Strehler layout
...
Now you need a class that will manage robots. Basic implementation of it is just an extension of Element with metaclass_data overridden. Class namespace is not important at all.
...
The relationship name in Category mdule. You can read it from Your::Module::Result::Category as here:
...
Hooks will not be used in this tutorial.
