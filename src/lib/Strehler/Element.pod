...
Retrieve the next/previous element of the same category using the display_order as ordering field.
...
=item get_last_by_order/get_first_by_order

Arguments: $category_id, $language

Return Value: $element

Class method. Retrieve the last/first element in the category with category id $category_id. Ordering using display_order.
...
Available only for articles. Retrive the Article $element using the slug generated in automatic by CMS.
